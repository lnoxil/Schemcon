...
def build_mapping(source_blocks: set[str], target_blocks: set[str]) -> dict[str, str]:
    """Создаёт базовый маппинг блоков."""
    mapping = {}
...
def build_smart_mapping(source_blocks: set[str], target_blocks: set[str]) -> dict[str, dict]:
    """НОВОЕ: Создаёт умный маппинг с метаданными о качестве замены."""
    mapping = {}
    for block in sorted(source_blocks):
        result = pick_best_match(block, target_blocks)
        mapping[block] = {
...
def apply_mapping_to_palette(
    ...
    """Применяет маппинг к палитре блоков."""
    new_palette = {}
    warnings = []
    used_targets = {}
...
def apply_smart_mapping_to_palette(
    ...
    """НОВОЕ: Применяет умный маппинг с сохранением метаданных."""
    new_palette = {}
    warnings = []
    replacements = []
    used_targets = {}
    ...
    for name, index in palette.items():
        ...
        if existing is not None and existing != index:
            ...
            target = name  # Оставляем оригинал
            mapping_info = {
                "target": name,
...
def convert_schematic_smart(
    ...
    """НОВОЕ: Умная конвертация с детальными отчётами."""
    schematic = load_schematic(input_path)
    palette = schematic.palette
    ...
    # Группировка по уровню уверенности
    confidence_groups = {
        "perfect": [],      # 1.0
        "high": [],         # 0.8-0.99
        "medium": [],       # 0.5-0.79
