...
BIOME_TINT_PLAINS = (91, 181, 47)
WATER_TINT = (63, 118, 228)


def _apply_tint(color: tuple[int, int, int], tint: tuple[int, int, int]) -> tuple[int, int, int]:
    return (
        (color[0] * tint[0]) // 255,
        (color[1] * tint[1]) // 255,
        (color[2] * tint[2]) // 255,
...
def _is_tinted_block(base_name: str) -> bool:
    return any(h in base_name for h in TINTED_BLOCK_HINTS)
    ...
    with zipfile.ZipFile(client_jar) as jar:
        ...
        for block_name in block_names:
            ...
            if weighted_colors:
                ...
                # Many foliage textures are grayscale and require biome tint;
                # apply stable plains tint so gradients are not washed out/white.
                if _is_tinted_block(base):
                    color = _apply_tint(color, BIOME_TINT_PLAINS)

                if "water" in base or "bubble_column" in base:
                    color = _apply_tint(color, WATER_TINT)
